...
export class AutoGeneratedFormSectionComponent {
  ...
  ngOnInit(): void {
    if (this.section) {
      ...
      for (let field of this.section.fields) {
        /**
         * creo un FormControl per ogni field della sezione
         */
        const fc = new FormControl(this.getFieldValue(field), this.getValidators(field));
      ...
      for (let field of this.section.fields) {
        ...
        /**
         * se il campo appena inserito nel FormControl dipende da altri campi
         */
        if (field.depends?.isDependent) {
          ...
          for (let campo of this.section.fields) {
            if (field.depends?.isDependent && field.depends.dependsFrom?.includes(campo.fieldName)) {
          ...
          for (let campo of dependsOn) {
            ...
            merge(thisField$, questoField$).subscribe(func => {
              this.console.log(func)
              let isValid: boolean = true;
              if (this.formArray.at(campo.order).invalid) {
                isValid = false;
              }

              //controllo che tutti i campi da cui dipende siano validi
  ...
  public fieldsSorter(section: Section): Section {
    section.fields.sort(function (a, b): number { return a.order - b.order; });
    return section;
  }
   ...
   * @returns 
   */
  public textValidators(field: Field, validatorsArray: ValidatorFn[]): ValidatorFn[] {
  ...
  /**
   * riassegna i valori iniziali al formArray  
